...
def train_link_prediction(args):
    ...
    A_full = build_adjacency(full_data.sources, full_data.destinations, num_nodes).to(device)
    # For training, use training edges (subset of A_full).
    A_train = A_full.clone()
    
    # Create an edge dataset and DataLoader for training.
    train_edge_dataset = EdgeDataset(train_sources, train_destinations, train_timestamps)
    train_edge_loader = DataLoader(train_edge_dataset, batch_size=args.batch_size, shuffle=True)
    ...
    for epoch in range(num_epochs):
        ...
        for batch in train_edge_loader:
            ...
            pos_scores = (embeddings[batch_sources] * embeddings[batch_destinations]).sum(dim=1)
            # Sample negative edges for this mini-batch.
            neg_edges = sample_negative_edges(batch_sources.cpu().numpy(),
